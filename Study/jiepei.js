/*
 * @Author: Sliverkiss
 * @Date: 2023-05-30 20:24:18
 * @Description:
 * 捷配 签到获得余额，可以用来打板
 * 抓Authorization,填写到jp_data中，多账号用换行
 * 
...
async function main() {
    ...
    for (let ck of user_ck) {
        ...
        let user = {
            index: index,
            cookie: ck //简写法, 效果等同于 openid: openid,
        };
...
function signin(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://abpmbapi.jiepei.com/api/applet/appletaccount/post-applet-signin?userId=262438",
            headers: header,
        };
        $.post(signinRequest, (error, response, data) => {
            try {
                ...
                console.log(data)
                if (result?.success) {
                    message += `签到成功！获得${result?.message}\n`;
                } else {
