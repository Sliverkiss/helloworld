/*
 * @Author: Sliverkiss
 * @Date: 2023-05-31 18:31:22
 * @FilePath: https://github.com/Sliverkiss/
 * @Description:
 * 微信小程序 三只松鼠小程序 每日签到
 * 捉h5.youzan.com域名任意包下的Cookie中的KDTWEAPPSESSIONID=XXXXXXXXXX,填写到szss_cookie中,多帐号换行
 * 
 * 只用过loon，理论上支持qx、surge，请自行尝试
 * 
 * [Script]
 * cron "0 35 1 * * *" script-path=https://raw.githubsercontent.com/Sliverkiss/helloworld/master/Study/szss.js, timeout=300, tag=三只松鼠
 * 
 */

const $ = new Env("三只松鼠");
const env_name = "szss_cookie";
...
function checkin(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://h5.youzan.com/wscump/checkin/checkinV2.json?checkinId=9332",
            headers: header,
        };
...
async function getCustomerPoints(user) {
    return new Promise((resolve) => {
        ...
        const signinRequest = {
            url: "https://h5.youzan.com/wscump/pointstore/getCustomerPoints.json?withNearlyExpired=true",
            headers: header,
        };
        $.get(signinRequest, (error, response, data) => {
          try{
