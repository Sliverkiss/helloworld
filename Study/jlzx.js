/*
 * @Author: Sliverkiss
 * @Date: 2023-05-28 22:39:31
 * @Description:
 * 江玲智行app 每日签到 积分可以兑换实物
 * 捉superapp.jmc.com.cn域名任意包下的Access-Token,填写到jlzx_data中,多帐号换行
 * 
        ...
        $.post(signinRequest, (error, response, data) => {
            try {
                ...
                if (result?.resultCode == 0) {
                    message += `🟢签到成功！${result?.resultMsg}\n`;
                } else {
                    message += `🟡签到失败！${result?.resultMsg}\n`
                }
            } catch (error) {
                message += `🔴${response}\n`;
                $.logErr(error)
...
//用户状态
async function getMenberInfo(user) {
    return new Promise((resolve) => {
        const header = {
            "Host": "superapp.jmc.com.cn",
            "Content-Length": "33",
        ...
        $.get(signinRequest, (error, response, data) => {
            try {
                var result = JSON.parse(data);
                if (result?.resultCode == 0) {
                    message += `🟢查询成功！总积分：${result?.data?.integralQuantity}\n`;
                } else {
                    message += `🟡查询失败！${result?.resultMsg}\n`
                }
            } catch (error) {
                message += `🔴${error}\n`;
                $.logErr(error)
